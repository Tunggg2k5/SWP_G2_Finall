...
export async function deleteAppointmentCascade(appointment) {
  ...
  const records = await findMany(COLLECTIONS.treatmentRecords, { appointment: appointmentId }, { projection: "_id" });
  ...
  await Promise.all([
    ...
    invoiceIds.length
      ...
      : Promise.resolve(),
    recordIds.length
      ? getCollection(COLLECTIONS.prescriptions).deleteMany({ treatmentRecord: { $in: recordIds } })
      : Promise.resolve(),
    recordIds.length
      ? getCollection(COLLECTIONS.treatmentPlans).deleteMany({ treatmentRecord: { $in: recordIds } })
      : Promise.resolve(),
